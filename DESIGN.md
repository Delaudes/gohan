...
Toggle d'affichage à côté du compteur (ex. masquer les repas déjà réalisés/ingrédients déjà achetés) : switch pilule, différent de la bascule à cocher ci-dessus (celle-ci change un affichage global de la liste, pas l'état d'un item) — piste `h-6 w-11 rounded-full`, `bg-charcoal/20` au repos / `bg-tomato` activé, pastille `h-5 w-5 rounded-full bg-cream` qui glisse (`translate-x-5` activé). Implémenté via `<input type="checkbox" class="sr-only peer">` + classes `peer-checked:` sur les éléments visuels, pas de composant dédié — même logique qu'ailleurs dans l'app : utiliser l'élément natif directement plutôt qu'un composant wrapper dédié. Label texte à droite du switch, `text-charcoal/50 font-mono text-xs`. Skeleton associé : piste `h-6 w-11` + texte `h-4 w-20`, groupés dans le même conteneur flex que le vrai switch+label.
...
Exception au dialog de création : quand la confirmation visuelle immédiate compte plus que la légèreté d'un FAB (ex. ajouter un ingrédient existant à une recette/liste de courses, un repas à la liste de repas), pas de `<dialog>` — le FAB lui-même se transforme en champ de recherche plutôt que d'en ouvrir un par-dessus :
replié, cercle `h-14 w-14` identique au FAB de création (même position `fixed right-4 bottom-[calc(6rem+env(safe-area-inset-bottom))]`, bordure, ombre, effet hover qui "enfonce" le bouton) ;
déplié (tap), la largeur anime jusqu'à quasi pleine largeur (`calc(100vw - 2rem)`, `transition-all`) et le fond bascule `bg-tomato` → `bg-cream`, révélant une icône loupe (`search`, `text-tomato`) + un champ souligné (même style que le champ de création) + un bouton de fermeture (`close`, `text-charcoal/50`) — l'effet hover (ombre qui rétrécit + translation) ne s'applique qu'à l'état replié, pas une fois déplié.
Les résultats/actions ne poussent jamais le champ : ils s'affichent dans un panneau flottant (`absolute right-0 bottom-full left-0`, ancré sur le cercle/pilule) qui recouvre le contenu **au-dessus** du champ, jamais en dessous — sur mobile le clavier masquerait sinon les résultats, et un panneau qui pousse le champ plutôt que de flotter par-dessus finit par le faire sortir de l'écran visible pendant la frappe. N'apparaît que si le champ est non vide, à l'intérieur de l'état déplié — pas de condition combinée en une seule expression, un `@if` imbriqué dans l'autre.

Champ de recherche live (ex. rechercher un ingrédient à ajouter) : même style de champ souligné que la création, avec une icône loupe (`search`, `text-tomato`) — signale que la saisie filtre du contenu en direct, contrairement au champ de création simple (confirmé par un bouton, pas de filtrage en direct).
Champ de recherche d'une liste complète affichée sur la page (ex. filtrer les ingrédients/recettes visibles) : `sticky top-0` en tête de page plutôt que fixe en bas — un clavier mobile qui remonte ne masque jamais un champ ancré en haut. Barre plate sans traitement carte (`bg-cream p-3`, pas de bordure ni d'ombre), pleine largeur via marge négative (`-mx-4 -mt-4`) pour rester flush avec les bords du conteneur `p-4` de la page.
