...
Badge de statut sous un nom d'item (ex. "Dans la liste de repas/de courses", "Repas réalisé"/"Acheté") : `font-mono text-tomato text-[11px] font-medium` — toujours un texte pour les deux états (fait/pas fait, acheté/pas acheté), jamais un espace vide en creux ; l'état négatif passe en `text-charcoal/40` plutôt que `text-tomato` (réservé à l'état positif). Deux façons de l'aligner sous le nom selon la structure de la ligne : soit imbriqué dans la même colonne flex que le nom (le décalage vient naturellement du bouton de bascule placé avant — cas le plus courant), soit — si le badge est sur une ligne séparée en dessous (ex. carte recette) — un retrait explicite (`pl-14`) pour compenser la largeur du bouton à gauche (`h-11 w-11` + `gap-3`). Toujours réserver l'espace — hauteur minimale ou élément vide rendu plutôt qu'un `@if` sans `@else` — pour ne pas décaler les lignes/le skeleton quand le badge n'est pas affiché.
