...
src/<feature>/
  ...
  usecases/<action>.usecase.ts          <Action>UseCase(view, port).execute() : loading → appel port → present ou error → loading off
                                         ...
                                         échec métier attendu à distinguer d'un échec technique générique (ex. suppression refusée par une règle business) → le port renvoie `Promise<<Action>Result>` (`{success:true} | {success:false, error: <Action>Error}`) au lieu de throw pour cet appel précis ; l'adapter catch tout en interne et ne rejette jamais pour cette méthode, donc pas de try/catch dans le usecase autour de cet appel. Le usecase reste le seul endroit qui importe à la fois `<Action>Error` (domain model) et le texte à afficher — `Record<<Action>Error, <Action>ErrorMessage>` exhaustif — puis passe le message déjà résolu à `view.update(vm => vm.presentErrorX(id, message))` ; le view model stocke/expose ce message tel quel, sans jamais importer `<Action>Error` lui-même
  ...
  models/<feature>.api.model.ts         forme API, mappée vers le domain model par l'adapter — vit avec les autres models de la feature, pas sous adapters/
                                         un champ déclaré ici mais jamais lu par le mapping vers le domain model ne devrait pas y figurer — pas un miroir complet de la réponse API si certains champs ne servent à rien dans cette feature ; deux endpoints qui renvoient la même ressource dans deux features différentes peuvent donc avoir deux `*ApiModel` de formes différentes (chacun garde seulement ce qu'il mappe)
                                         un champ qui peut manquer côté API : `| null` si l'API renvoie littéralement `null` sur le fil (JSON ne transforme jamais un `null` en clé absente), `?:` seulement si la clé peut être réellement absente de la réponse — la conversion `null` → `undefined` vers le domain model se fait explicitement dans l'adapter (`?? undefined`), jamais en mentant sur la forme réelle du JSON dans le typage de l'api model
  adapters/
...
Composants de layout globaux (header, footer, navigation) : suffixe `.component`, préfixés `app.`, chacun dans son propre dossier sous `src/app/` — ex. `src/app/header/app.header.component.ts` → `AppHeaderComponent`. Le composant racine (`app.component.ts`) reste à plat dans `src/app/`, pas de dossier dédié pour lui.

Préoccupations transverses/techniques (pas liées à une feature métier) → `src/infra/`, même principe port/adapter, un dossier par concern (`infra/http/`, `infra/storage/`, `infra/route/`) : `<domain>.port.ts`, `<impl>-<domain>.adapter.ts` (+ une variante `Fake<Domain>Adapter` pour les tests), `<domain>.provider.ts` exposant un `InjectionToken` (`providedIn:'root'`, factory qui instancie l'implémentation active). Les adapters récupèrent leurs dépendances via `inject()` en initialiseur de champ, pas par constructeur.
Exception : l'état réactif n'a pas de port dédié (`infra/signal/`) — `<feature>.view.ts` utilise `signal()` d'Angular directement. Le port existait pour permettre de swapper l'implémentation, mais avec un seul moteur réactif réel (Angular signals, testable tel quel sans `TestBed`) l'abstraction ne protégeait contre rien.
...
Préoccupations transverses côté UI, réutilisables entre features → `src/shared/`, sans port ni provider (rien à swapper, juste du comportement générique) :
`shared/directives/<action>-when.directive.ts` : `selector: '[app<Action>When]'`, un seul `input.required<boolean>()` et un unique `effect()` qui agit sur l'`ElementRef` natif quand la valeur passe à `true` (focus, vidage de champ, fermeture de dialog...). Une seule action par directive — ne pas faire un `if/else` avec deux actions différentes dans le même `effect()`, plusieurs directives se posent côte à côte sur le même élément quand il en faut plusieurs. Déclenchée depuis le template par un champ `isSuccessX`/`isXVisible` du view model (jamais une expression composée du genre `vm.a && vm.b` inline dans le template — si deux conditions doivent se combiner pour l'affichage, imbriquer deux `@if` plutôt que les combiner en une seule).
`shared/utils/<nom>.utils.ts` : fonctions pures, sans état ni port, importables depuis n'importe quelle couche d'une feature — usecases, view models, composants (ex. normalisation de texte pour la recherche).

Pas de `FormsModule`/formulaires réactifs : un input se lit via sa référence de template (`#ref`), sa valeur (`.value`) lue et passée en primitive au usecase — pas de state Angular intermédiaire, pas de `<form>`/`(submit)`, pas de gestion d'Enter (bouton uniquement).
