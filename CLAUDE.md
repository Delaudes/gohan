...
Une feature peut avoir plusieurs ports quand un sous-domaine mérite d'être isolé (ex. `RecipesPort` pour le CRUD recette + `RecipeIngredientsPort` pour la recherche/l'attachement d'ingrédients à une recette) — chacun son adapter et son `InjectionToken`, mais un seul `<feature>.provider.ts`/`<FEATURE>_PROVIDERS` partagé. Idem pour les views : une feature avec une liste ET une fiche détail peut avoir `<Feature>View` (pluriel, état liste) et `<Item>View` (singulier, état détail plus riche avec ses propres `isLoadingX`), chacune son `<Item>ViewModel`, toutes deux instanciées dans le même `<FEATURE>_PROVIDERS`.
