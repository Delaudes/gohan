...
- Un seul `it()` par getter, jamais un par sous-cas. Les différents sous-cas d'un même getter (vide/rempli, query vide/partielle/sans résultat, singulier/pluriel...) s'enchaînent dans le MÊME test, chaque assertion servant de "avant" à la transition suivante — pas un `it()` séparé par sous-cas. Le titre doit alors couvrir l'intégralité de ce qui est enchaîné dans le test (ex. `'should match an available option by a partial, diacritic-insensitive query, or nothing when the query is empty or unmatched'`), jamais nommé d'après uniquement le premier sous-cas.
- Les données de test doivent réellement discriminer le comportement, pas coïncidentiellement passer :
  ...
  - un test d'insensibilité aux diacritiques doit mettre l'accent dans la QUERY (pas seulement dans le nom stocké) — si le nom cible n'a lui-même aucun accent, un test qui n'accentue que le nom ne prouve rien sur le folding de la query, alors que c'est elle qui vient de la saisie utilisateur ;
...
- Un getter qui dérive d'un AUTRE getter du même ViewModel (ex. `matchingX` qui filtre `availableX`, lui-même dérivé de la liste courante) : le test de `matchingX` ne doit pas réutiliser le fixture peuplé du test de `availableX` — l'isoler avec un état minimal (liste vide) qui laisse `availableX` renvoyer tout, pour que seule la logique propre à `matchingX` soit exercée. Sinon un bug dans la dérivation intermédiaire fait échouer les deux tests à la fois, pour des raisons différentes, et le diagnostic devient confus.
